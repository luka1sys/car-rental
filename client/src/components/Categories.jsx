const Categories = () => {
    const categoryData = [
        { title: 'Sport Car', img: '/luxury-collection-img-1.jpg', delay: 'dvv1' },
        { title: 'Convertible Car', img: '/luxury-collection-img-2.webp', delay: 'dvv2' },
        { title: 'Sedan Car', img: '/luxury-collection-img-3.webp', delay: 'dvv3' },
        { title: 'Luxury car', img: '/luxury-collection-img-4.webp', delay: 'dvv4' },
    ];

    return (
        <section className='w-full max-w-[97%] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-[10px] px-4 sm:px-6 md:px-8'>
            {categoryData.map((item, index) => (
                <div
                    key={index}
                    className={`group relative rounded-[30px] sm:rounded-[40px] overflow-hidden cursor-pointer h-[250px] sm:h-[300px] md:h-[350px] lg:h-[450px] ${item.delay} animate-on-scroll`}
                >
                    {/* სურათის ფენა - ეს ნაწილი დიდდება */}
                    <div
                    ...
                    <div className='absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors duration-500'>
                        ...
                        {/* ღილაკი */}
                        <button className="absolute bottom-6 sm:bottom-8 md:bottom-[40px] right-6 sm:right-8 md:right-[40px] bg-[rgb(255,54,0)] w-[45px] h-[45px] sm:w-[50px] sm:h-[50px] md:w-[55px] md:h-[55px] rounded-full group-hover:bg-black transform rotate-[-45deg] group-hover:rotate-[0deg] transition-all duration-500 flex items-center justify-center">
